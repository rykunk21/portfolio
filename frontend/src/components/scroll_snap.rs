    ...
    
    {
        ...
        use_effect_with((), move |_| {
            ...
            let closure = Closure::wrap(Box::new(move || {
                if let Ok(scroll_y) = window_for_closure.scroll_y() {
            ...
            let _ = window_for_closure.add_event_listener_with_callback("scroll", closure.as_ref().unchecked_ref());
            closure.forget();
        });
    }
                ...
                { for (0..3).map(|i| {
                    ...
                    html! {
                        <div class={if is_active { "w-3 h-3 rounded-full" } else { "w-2 h-2 rounded-full opacity-50" }}
                             style={if is_active { 
                                 "background: var(--color-highlight-500); box-shadow: 0 0 10px var(--color-highlight-500);"
                             } else {
