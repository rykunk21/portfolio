...
// Pre-generate star positions once (static random)
fn generate_stars_static(count: usize, max_size: usize, min_opacity: f32, max_opacity: f32) -> Vec<(String, String)> {
...
pub fn background(props: &BackgroundProps) -> Html {
    ...
    // Generate stars ONCE on mount, cache them
    let stars_layer1 = use_state(|| generate_stars_static(200, 1, 0.25, 0.45));
    let stars_layer2 = use_state(|| generate_stars_static(150, 2, 0.3, 0.55));
    let stars_layer3 = use_state(|| generate_stars_static(100, 3, 0.35, 0.65));
    let stars_layer4 = use_state(|| generate_stars_static(50, 4, 0.5, 0.9));
    ...
    html! {
        <div class="relative min-h-screen overflow-x-hidden">
            ...
            // Star Layer 1: Deepest (200 stars)
            <div class="fixed inset-0 pointer-events-none"
                style={format!("transform: translateY({}px);", star_layer1_offset)}>
                { for stars_layer1.iter().map(|(style, key)| {
                    html! { <div class="star-dim" style={style.clone()} key={key.clone()} /> }
                })}
            </div>
            ...
            <div class="fixed inset-0 pointer-events-none"
                ...
                { for stars_layer2.iter().map(|(style, key)| {
                    html! { <div class="star-far" style={style.clone()} key={key.clone()} /> }
                })}
            </div>
            ...
            <div class="fixed inset-0 pointer-events-none"
                ...
                { for stars_layer3.iter().map(|(style, key)| {
                    html! { <div class="star-mid" style={style.clone()} key={key.clone()} /> }
                })}
            </div>
            
            // Star Layer 4: Near (50 stars)
            <div class="fixed inset-0 pointer-events-none"
                style={format!("transform: translateY({}px);", star_layer4_offset)}>
                { for stars_layer4.iter().map(|(style, key)| {
                    html! { <div class="star-near" style={style.clone()} key={key.clone()} /> }
                })}
            </div>
